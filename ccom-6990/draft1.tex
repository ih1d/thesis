...
room for improvement. In fact, systems like TensorFlow are rooted in PL concepts.\cite{tfmodel} Today, 
ML libraries have been extended permiting programmers to explicitly use dataflow semantics changing from 
the usual imperative semantics. However, this conversion from imperative to dataflow has proven to be challenging 
for programmers, primarily looking to optimize their code, leading to bugs or performance issues (the opposite of 
what the programmer intended to do).\cite{imptograph} In fact, these extensions are so pervasive that they even 
alter the execution and modification of state!\cite{tfmodel} Two surveys (\cite{impact},~\cite{imptograph}) give a 
taxonomy of bugs which in some way correlates to programming languages. The cause of bugs can range from lack of 
features in a language (e.g.\ lack of type system guarantees) to a misunderstanding of the change of semantics 
from imperative to graph execution.

The argument is that a reflective language can help a programmer understand dataflow semantics by allowing
...
\section{ML, PL and Bugs}

The two surveys mentioned in the introduction (\cite{impact},~\cite{imptograph}) analyze bugs
in ML programs. The surveys categorize the bugs and give examples of each category. Both surveys
ask a total of 5 research questions (RQs). For the first survey (\cite{imptograph}), the RQs are:
\begin{itemize}
    \item RQ1: What bug patterns and corresponding challenges are involved in writing reliable yet performant imperative DL code?
    \item RQ2: Which best practices and anti-patterns can be extracted from (RQ1)
\end{itemize}

For the second survey (\cite{impact}), the RQs are:
\begin{itemize}
    \item RQ1: Do the bug characteristics depend on the chosen programming language?
    \item RQ2: Does the application domain influence the bug characteristics within a chosen 
    programming language?
    \item RQ3: Are differences in the bug distribution explainable by the features of the 
